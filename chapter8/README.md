# sLLM 서빙하기  
앞에서는 성능을 희생해 효율성을 높였다면 여기서는 동일한 연산을 수행해 성능 하락이 없으면서도 추론 속도를 높이는 방법들을 알아본다.  
...
6. 더 작은 드래프트 모델과 더 성능이 좋은 타깃 모델을 사용해 2~3배 높이는 추측 디코딩 방식
7. vLLM을 활용해 오프라인 추론과 온라인 추론을 수행하는 방법 
